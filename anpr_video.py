...
def start_anpr(input_files):
    ...
    model = OCR(
        modelFile=r"C:\Users\mgree\Downloads\ANPR_ATCC_SMART_TRAFFIC_MANAGEMENT (2)\ANPR_ATCC_SMART_TRAFFIC_MANAGEMENT\models\binary_128_0.50_ver3.pb",
        labelFile=r"C:\Users\mgree\Downloads\ANPR_ATCC_SMART_TRAFFIC_MANAGEMENT (2)\ANPR_ATCC_SMART_TRAFFIC_MANAGEMENT\models\binary_128_0.50_labels_ver2.txt"
    )
    ...
    for file_path in input_files:  # Iterate over the file paths
        ...
        cv2.resizeWindow('original video', 500, 500)
        ...
        while cap.isOpened():
            ...
            if ret:
                ...
                if possible_plates is not None:
                    for i, p in enumerate(possible_plates):  # Loop through detected plates
                        ...
                        # Convert the plate image to base64
                        _, buffer = cv2.imencode('.jpg', p)
                        plate_image_base64 = base64.b64encode(buffer).decode('utf-8')

                        # Store number plate text & image in DB
                        connection = dbconnection()

                        with connection.cursor() as cursor:
                            sql_query = """INSERT INTO vehicle_data (number_plate_text, plate_image_base64 , road_id ,violation) 
                                           VALUES (%s, %s, %s , %s)"""
                            cursor.execute(sql_query, (recognized_plate, plate_image_base64 , 1 , 0))
                            connection.commit()
        ...
        cap.release()  # Release video capture for each file
