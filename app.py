...
def dbconnection():
     connection = pymysql.connect(host='localhost',database='traffic management system',user='root',password='greesh09@25M')
     return connection
...
def show_results():
    connection = dbconnection()
    print(f"Database Connection: {connection}")  # Debugging
    results = []

    with connection.cursor(pymysql.cursors.DictCursor) as cursor:
        # Fetching all results sorted by timestamp (assuming timestamp field exists)
...
def accident_detection():
    ...
    try:
        ...
        model_path = r"C:\Users\mgree\Downloads\ANPR_ATCC_SMART_TRAFFIC_MANAGEMENT (2)\ANPR_ATCC_SMART_TRAFFIC_MANAGEMENT\models\best.pt"  # Use your YOLO model path
